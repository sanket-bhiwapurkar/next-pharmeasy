  ...
  return (
    <main className="flex flex-col min-h-screen">
      ...
      <section className="py-8 px-4 flex flex-col mt-6 gap-4 w-full items-center">
        ...
        />
        <LightSlick>
          {offeringsData.map((eachOffering) => (
            <OfferingsCard
              key={eachOffering.id}
              imgUrl={eachOffering.imgUrl}
      ...
      <section className="py-8 px-4 flex flex-col gap-8 w-full">
        ...
        />
        <LightSlick>
          {pairArray(labTestsData).map((eachTest, i) => (
            <div key={i} className="flex flex-col md:flex-row gap-4">
              <BasicCard
                imgUrl={eachTest[0].imgUrl}
                className="min-w-36 xl:min-w-44"
              />
              <BasicCard
                imgUrl={eachTest[1].imgUrl}
                className="min-w-36 xl:min-w-44"
              />
            </div>
          ))}
      ...
      {/* Categories */}
      <section className="py-8 px-4 flex flex-col gap-8 w-full">
        <SectionHeader head="Shop by Categories" />
        <LightSlick>
          {pairArray(categoriesData).map((eachTest, i) => (
            <div key={i} className="flex flex-col md:flex-row gap-4">
              <BasicCard
                imgUrl={eachTest[0].imgUrl}
                title={eachTest[0].title}
                className="min-w-36 xl:min-w-44"
              />
              <BasicCard
                imgUrl={eachTest[1].imgUrl}
                title={eachTest[1].title}
                className="min-w-36 xl:min-w-44"
              />
            </div>
          ))}
      ...
      <section className="py-8 px-4 flex flex-col gap-8 w-full">
        ...
        />
        <LightSlick>
          {pairArray(productsData).map((eachProduct, i) => (
            <div key={i} className="flex flex-col gap-4">
              <ProductBetaCard
                key={eachProduct[0].id}
                orientation="horizontal"
                imgUrl={eachProduct[0].imgUrl}
                title={eachProduct[0].title}
                mp={eachProduct[0].mp}
                sp={eachProduct[0].sp}
                discount={eachProduct[0].discount}
              />
              <ProductBetaCard
                key={eachProduct[1].id}
                orientation="horizontal"
                imgUrl={eachProduct[1].imgUrl}
                title={eachProduct[1].title}
                mp={eachProduct[1].mp}
                sp={eachProduct[1].sp}
                discount={eachProduct[1].discount}
              />
            </div>
          ))}
        </LightSlick>
      ...
      <section className="py-8 px-4 flex flex-col gap-8 w-full">
        ...
        />
        <LightSlick>
          {featuredBrandsData.map((eachProduct, i) => (
            <BasicCard
              key={eachProduct.id}
              imgUrl={eachProduct.imgUrl}
              title={eachProduct.title}
              className="min-w-24 lg:min-w-44"
            />
          ))}
        </LightSlick>
      </section>
      ...
      {/* Deals of the Day */}
      <section className="py-8 px-4 flex flex-col gap-8 w-full">
        <SectionHeader head="Deals of the Day" />
        <LightSlick>
          {productsData.map((eachProduct, i) => (
            <ProductBetaCard
              key={eachProduct.id}
      ...
      <section className="bg-gradient-to-b from-[#fff8e3] to-transparent mt-8">
        <div className="py-8 px-4 flex flex-col gap-8 w-full bg-[url('/images/bg/spotlight.svg')] bg-contain bg-center bg-no-repeat ">
          <SectionHeader head="In the Spotlight" />
          <LightSlick>
            {productsData.map((eachProduct, i) => (
              <ProductBetaCard
                key={eachProduct.id}
